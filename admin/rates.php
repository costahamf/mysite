...
function parseRatesBulkInput(string $input): array
{
    ...
    if ($rows) {
        return array_map(function(array $parts): array {
            // Автоматически умножаем max_income_per_courier на 0.8
            $maxIncome = (float) str_replace(',', '.', $parts[7]);
            $maxIncomeForPartner = $maxIncome * 0.8;
    ...
    foreach ($chunks as $chunk) {
        ...
        // Автоматически умножаем max_income_per_courier на 0.8
        $maxIncome = (float) str_replace(',', '.', $chunk[7]);
        $maxIncomeForPartner = $maxIncome * 0.8;
        ...
        $result[] = [
            ...
            'priority' => (float) str_replace(',', '.', $chunk[4]),
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        ...
        try {
            ...
            $insert = $pdo->prepare('INSERT INTO rates (city, lead_type, registered_from, registered_to, priority, cd_threshold, order_rate_over_cd, max_income_per_courier) VALUES (:city, :lead_type, :registered_from, :registered_to, :priority, :cd_threshold, :order_rate_over_cd, :max_income_per_courier)');
...
<div class="container py-4">
    ...
    <div class="crm-card p-4 mb-3">
        ...
        <div class="alert alert-info small">
            <strong>ℹ️ Важно:</strong> Макс. доход автоматически умножается на 0.8 (80% партнеру). Вводите полную сумму, система сама рассчитает долю партнера.
        </div>
...
26000.00"><?= h($bulkInput) ?></textarea>
    ...
    <div class="table-responsive crm-card p-3">
        <table class="table table-striped align-middle mb-0">
            ...
            <tr>
                ...
                <th>Макс. доход <span class="tooltip-icon" title="Уже умножено на 0.8 (80% партнеру)">ℹ️</span></th>
            </tr>
            </thead>
            <tbody>
            <?php if (!$rates): ?>
            ...
            <?php foreach ($rates as $row): ?>
                <tr>
                    ...
                    <td><?= number_format((float)$row['priority'], 0, ',', ' ') ?></td>
                    <td><?= number_format((float)$row['cd_threshold'], 0, ',', ' ') ?></td>
                    <td><?= number_format((float)$row['order_rate_over_cd'], 2, '.', ' ') ?></td>
                    <td><strong><?= number_format((float)$row['max_income_per_courier'], 2, '.', ' ') ?></strong> ₽</td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
    ...
    <div class="crm-card p-3 mt-3">
        <h3 class="h6 mb-2">Информация по расчетам:</h3>
        <p class="small text-muted mb-0">
            Макс. доход сохраняется в БД уже с учетом 80% доли партнера.
            Например, если вы ввели 26000, система сохранит 20800 (26000 × 0.8).
        </p>
    </div>
</div>
</body>
